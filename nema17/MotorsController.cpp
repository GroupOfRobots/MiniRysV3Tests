...
float MotorsController::getMotorSpeedLeft() const {
	...
	return this->motorSpeedLeft * (this->invertLeft ? -1 : 1) * PRU_CLOCK/(STEPPER_STEPS_PER_REVOLUTION * MAX_MOTOR_SPEED);
}
...
float MotorsController::getMotorSpeedRight() const {
	...
	return this->motorSpeedRight * (this->invertRight ? -1 : 1) * PRU_CLOCK/(STEPPER_STEPS_PER_REVOLUTION * MAX_MOTOR_SPEED);
}
